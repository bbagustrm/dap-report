            ...
            <div class="w-full px-8 mb-10">
                ...
                <div class="w-full bg-white px-4 py-4 text-sm shadow-sm rounded-md">
                    <form class="flex flex-col gap-4" action="{{ route('dailies.update', $daily->id) }}" method="POST">
                        ...
                        <trix-editor input="x"
                            class="min-h-[200px] px-4 py-2">{!! $daily->note !!}</trix-editor>
                        <hr>
                                    ...
                                    <tr>
                                        <td class="border border-gray-300 px-2 w-[8vw]">
                                            <div class="flex gap-2 items-center justify-center">
                                                @if ($report['tipe'] == 1)
                                                    <span
                                                        class="px-2 text-center border border-[#28a745] text-[#28a745] rounded-full">harian</span>
                                                @elseif ($report['tipe'] == 2)
                                                    <span
                                                        class="px-2 text-center border border-[#ffc107] text-[#ffc107] rounded-full">mingguan</span>
                                                @elseif ($report['tipe'] == 3)
                                                    <span
                                                        class="px-2 text-center border border-[#dc3545] text-[#dc3545] rounded-full">bulanan</span>
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="border border-gray-300 w-[100px]">
                                            <input type="hidden" name="reports[{{ $report['id_tugas'] }}][id_tugas]"
                                                value="{{ $report['id_tugas'] }}">
                                            @php
                                                $target = str_replace('%', '', $report['target']);
                                            @endphp
                                            <input class="w-full p-2 border-[1px] border-gray-400"
                                                name="reports[{{ $report['id_tugas'] }}][score]"
                                                placeholder="Masukan Score" type="number"
                                                value="{{ $report['score'] }}" min="0"
                                                max="{{ $target }}">
                                        </td>
